...
configureMainButton({
  text: "view cart",
  color: "#008000",
  onclick: mainButtonClickListener,
});
...
async function loadItems() {
  const response = await fetch("https://bentelega.ru/telegram-web-app/static/foodItems.json");
  ...
  items.forEach((item, index) => {
    ...
    const addItemButton = foodItem.querySelector(
      ".food-item__button[data-add]"
    );
    addItemButton.addEventListener("click", () =>
      addItemListener(foodItem, index)
    );
    const removeItemButton = foodItem.querySelector(
      ".food-item__button[data-remove]"
    );
    removeItemButton.addEventListener("click", () =>
      removeItemListener(foodItem, index)
    );
  });
}
loadItems();
...
function getCartItem(foodItem, foodItemId) {
  const existingCartItem = document.querySelector(
    `.cart-item[data-food-item-id="${foodItemId}"]`
  );
  if (existingCartItem) {
    ...
    cartItem = cartItems.querySelector(
      `.cart-item[data-food-item-id="${foodItemId}"]`
    );
    const cartItemAddButton = cartItem.querySelector(
    ...
    const cartItemRemoveButton = cartItem.querySelector(
...
function sortCart() {
  ...
  items.sort(
    (a, b) => parseInt(a.dataset.foodItemId) - parseInt(b.dataset.foodItemId)
  );
  cartItems.innerHTML = "";
...
function createCartItem(foodItem, foodItemId) {
  ...
  const foodItemPrice = foodItem.querySelector(".food-item__price");
  const cartItemPrice = cartItem.querySelector(".cart-item__price");
  cartItemPrice.textContent = formatter.format(
    parseFoodItemPrice(foodItemPrice.textContent) *
      parseInt(foodItem.dataset.count)
  );

  cartItem.querySelector(".cart-item").dataset.foodItemId =
    foodItemId.toString();
  return cartItem;
}
...
function updateItemsPrices(foodItem, cartItem) {
  ...
  cartItemPriceElement.textContent = formatter.format(
    foodItemPrice * foodItemCount
  );
  cartItemAmount.textContent = foodItem.dataset.count + "x";
}
